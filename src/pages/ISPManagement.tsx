...
export default function ISPManagement({ userRole = 'staff', userPermissions }: ISPManagementProps) {
  ...
  if (loading) return <div className="p-8 text-slate-500 dark:text-slate-400">Loading ISP accounts...</div>;

  return (
    <div className="p-4 sm:p-6 lg:p-8 overflow-y-auto flex-1">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2.5 flex-wrap">
            <span>ISP Management</span>
            <SyncStatusBadge onSynced={fetchAccounts} />
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Track branch ISPs, speeds, and downtime with local-first offline sync</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm"
            />
          </div>
          <button
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {filteredAccounts.map(acc => (
          <div key={acc.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm flex flex-col">
            <div className="bg-slate-50 dark:bg-slate-900 p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-800 dark:text-slate-200 tracking-tight text-lg">{acc.ispName}</h3>
                  <div className="flex items-center text-sm text-slate-500 dark:text-slate-400 gap-1 mt-0.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => deleteAccount(acc.id)} className={`text-slate-400 dark:text-slate-500 hover:text-red-600 transition-colors ${userRole === 'it_assistant' ? 'hidden' : ''}`}>
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-6 flex-1">
               <div>
                 <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1">Speed</p>
               ...
               <div>
                 <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1">User / Device ID</p>
                 <p className="font-mono text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 p-1.5 rounded">{acc.userIdDeviceId || 'N/A'}</p>
               </div>
               <div>
                 <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1">Contact</p>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 p-4">
               <div className="flex items-center justify-between mb-3">
                 <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider flex items-center gap-1">
                   <Clock className="w-4 h-4 text-slate-400 dark:text-slate-500" /> Downtime History
                 </h4>
                 {acc.downtimeRecords && acc.downtimeRecords.length > 0 && (
                   <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 rounded">
               ...
               {acc.downtimeRecords && acc.downtimeRecords.length > 0 ? (
                 <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
                   {[...acc.downtimeRecords].reverse().map((dt: any, revIdx: number) => {
                     ...
                     return (
                       <div key={actualIdx} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded p-2 text-xs group">
                         <div className="flex justify-between items-start mb-1">
                           <div>
                             <p className="font-semibold text-slate-700 dark:text-slate-300">{format(new Date(dt.date), 'MMM dd, h:mm a')}</p>
                             <p className="text-slate-500 dark:text-slate-400 mt-0.5">{dt.reason}</p>
                           </div>
                             <div className="flex flex-col items-end gap-1">
                               <div className="flex items-center gap-1">
                                 <span className={`text-[9px] font-bold uppercase px-1 rounded border ${dt.status === 'offline' ? 'bg-red-50 text-red-600 border-red-100' : dt.status === 'degraded' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-slate-50 text-slate-600 dark:text-slate-400 border-slate-100'}`}>
                                   {dt.status || 'OFFLINE'}
                                 </span>
                               ...
                               <button onClick={() => openEditDowntime(acc.id, actualIdx)} className={`text-[10px] bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-200 text-slate-600 dark:text-slate-400 px-1 rounded transition-colors ${(userRole === 'it_assistant' && isAdminAddedData(acc)) ? 'hidden' : ''}`}>Edit</button>
                               {isUnresolved && (
                         ...
                         {dt.resolvedAt && <p className="text-[10px] text-slate-400 dark:text-slate-500">Resolved at: {format(new Date(dt.resolvedAt), 'MMM dd, h:mm a')}</p>}
                       </div>
                     );
                   })}
                 </div>
               ) : (
                 <p className="text-xs text-slate-500 dark:text-slate-400 italic">No historical downtime recorded.</p>
               )}
            </div>
          </div>
        ))}
        ...
        {ispAccounts.length === 0 && (
          <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
             ...
             <p className="text-slate-500 dark:text-slate-400 font-medium">No ISP accounts tracked yet</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-200">{editingId ? 'Edit Connection' : 'New ISP Connection'}</h3>
              <button onClick={() => setShowModal(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleSave} className="p-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">ISP Name</label>
                  <input required type="text" value={newAccount.ispName} onChange={e => setNewAccount({...newAccount, ispName: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. Starlink, AT&T" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Branch Office Link</label>
                    <input required type="text" value={newAccount.branchOffice} onChange={e => setNewAccount({...newAccount, branchOffice: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. HQ, NY Office" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Current Status</label>
                    <select value={newAccount.currentStatus} onChange={e => setNewAccount({...newAccount, currentStatus: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Speed</label>
                    <input type="text" value={newAccount.speed} onChange={e => setNewAccount({...newAccount, speed: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. 500 Mbps" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">User / Device ID</label>
                    <input type="text" value={newAccount.userIdDeviceId} onChange={e => setNewAccount({...newAccount, userIdDeviceId: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="Acc No." />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Contact Person (For Complaints)</label>
                  <input type="text" value={newAccount.contactName} onChange={e => setNewAccount({...newAccount, contactName: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="Name & Phone" />
                </div>
              </div>
              <div className="mt-6 flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-800 transition-colors cursor-pointer">Cancel</button>
      ...
      {showDowntimeModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800 bg-red-50/50">
            ...
            <form onSubmit={handleAddDowntime} className="p-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Start Date & Time</label>
                  <input required type="datetime-local" value={newDowntime.date ? newDowntime.date.split('.')[0].slice(0, 16) : ''} onChange={e => setNewDowntime({...newDowntime, date: e.target.value ? new Date(e.target.value).toISOString() : ''})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Resolved At (Optional)</label>
                  <input type="datetime-local" value={newDowntime.resolvedAt ? newDowntime.resolvedAt.split('.')[0].slice(0, 16) : ''} onChange={e => setNewDowntime({...newDowntime, resolvedAt: e.target.value ? new Date(e.target.value).toISOString() : ''})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Update ISP Status To</label>
                  <select 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500"
                  >
                    <option value="offline">Offline</option>
                    <option value="degraded">Degraded</option>
                    <option value="maintenance">Maintenance</option>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Duration (e.g. 2.5 hours)</label>
                  <input type="text" value={newDowntime.duration} onChange={e => setNewDowntime({...newDowntime, duration: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" placeholder="e.g. 2.5 hours" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Reason</label>
                  <textarea required value={newDowntime.reason} onChange={e => setNewDowntime({...newDowntime, reason: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" rows={2} placeholder="Brief description of the outage..."></textarea>
                </div>
              </div>
              <div className="mt-6 flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4">
                <button
