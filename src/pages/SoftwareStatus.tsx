...
export default function SoftwareStatus({ userRole = 'staff', userPermissions }: SoftwareStatusProps) {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      default: return <ShieldAlert className="w-5 h-5 text-slate-500 dark:text-slate-400" />;
    }
  };

  if (loading) return <div className="p-8 text-slate-500 dark:text-slate-400">Loading licenses...</div>;
    ...
    <div className="p-4 sm:p-6 lg:p-8 overflow-y-auto flex-1">
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Manage domains, servers & Microsoft licenses with local-first offline sync</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {licenses.map(lic => {
          ...
          return (
            <div key={lic.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6 shadow-sm relative group">
               <div className="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                 <button onClick={() => openEdit(lic)} className={`text-slate-400 dark:text-slate-500 hover:text-blue-600 transition-colors ${(userRole === 'it_assistant' && isAdminAddedData(lic)) ? 'hidden' : ''}`}>
                   <Edit2 className="w-4 h-4" />
                 </button>
                 <button onClick={() => handleDelete(lic.id)} className={`text-slate-400 dark:text-slate-500 hover:text-red-600 transition-colors ${userRole === 'it_assistant' ? 'hidden' : ''}`}>
               ...
               <div className="flex items-center gap-3 mb-4">
                 ...
                 <div>
                   <h3 className="font-bold text-slate-800 dark:text-slate-200 tracking-tight">{lic.name}</h3>
                   <p className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">{lic.type.replace('_', ' ')}</p>
                 </div>
               </div>
               ...
               <div className="space-y-3">
                 ...
                 {lic.notes && (
                   <p className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 p-2 rounded border border-slate-100 dark:border-slate-800">
                     {lic.notes}
                   </p>
                 )}
                 {lic.details && (
                   <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-1">
                     ...
                     <p className="text-xs text-slate-600 dark:text-slate-400"><span className="font-semibold">Details:</span> {lic.details}</p>
                   </div>
                 )}
               </div>
            </div>
        ...
        {licenses.length === 0 && (
          <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
             ...
             <p className="text-slate-500 dark:text-slate-400 font-medium">No licenses or domains tracked yet</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-200">{editingId ? 'Edit Item' : 'New Tracker Item'}</h3>
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input required type="text" value={newLicense.name} onChange={e => setNewLicense({...newLicense, name: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. mtknitsm.com" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Type</label>
                  <select value={newLicense.type} onChange={e => setNewLicense({...newLicense, type: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Expiry Date</label>
                  <input required type="date" value={newLicense.expiryDate.split('T')[0]} onChange={e => setNewLicense({...newLicense, expiryDate: new Date(e.target.value).toISOString()})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Notes / URL</label>
                  <textarea value={newLicense.notes} onChange={e => setNewLicense({...newLicense, notes: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" rows={2}></textarea>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Details / Description</label>
                  <textarea value={newLicense.details} onChange={e => setNewLicense({...newLicense, details: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" rows={2}></textarea>
                </div>
              </div>
