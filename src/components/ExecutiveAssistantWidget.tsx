...
export function ExecutiveAssistantWidget() {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden flex flex-col h-[520px]">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-sm tracking-tight text-white flex items-center gap-1.5">
              ...
              <span className="bg-emerald-500/20 text-emerald-400 text-[10px] px-1.5 py-0.5 rounded font-mono uppercase border border-emerald-500/30">AI Active</span>
            </h3>
            <p className="text-[11px] text-slate-400">Ask how to use any system module or metric</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-slate-900/50">
        {messages.map((msg, index) => (
          ...
          >
            ...
            <div
              className={cn(
                "max-w-[80%] rounded-2xl px-4 py-3 shadow-sm text-xs leading-relaxed",
                msg.role === 'user'
      ...
      {messages.length <= 2 && (
        <div className="px-4 py-2 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex flex-wrap gap-1.5 shrink-0">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider w-full mb-0.5">Quick Guide Questions:</span>
          {QUICK_QUESTIONS.map((q, idx) => (
            <button
              key={idx}
              onClick={() => handleSendMessage(q)}
              className="text-[11px] bg-slate-100 dark:bg-slate-700 hover:bg-blue-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-600 transition-colors text-left flex items-center gap-1 cursor-pointer"
            >
              <span>{q}</span>
